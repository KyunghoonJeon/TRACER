...
def main():
    ...
    new_kg_path = kg_path.replace('kg_raw', 'kg_refined')
    new_kg_triples = set()
        ...
        with open(new_kg_path, 'w') as f:
            with open(kg_path, 'r') as kg_file:
                for line in kg_file:
                    ...
                    if new_head.startswith('''"''') and new_head.endswith('''"'''):
                        new_head = new_head[1:-1]
                    if new_tail.startswith('''"''') and new_tail.endswith('''"'''):
                        new_tail = new_tail[1:-1]
                    if new_relation.startswith('''"''') and new_relation.endswith('''"'''):
                        new_relation = new_relation[1:-1]
