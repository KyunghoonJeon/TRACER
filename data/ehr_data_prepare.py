...
from utils import mortality_prediction_mimic3_fn, readmission_prediction_mimic3_fn, mortality_prediction_mimic4_fn, readmission_prediction_mimic4_fn
import csv
import pickle
...
from pyhealth.datasets import sample_dataset
    ...
    for patient, idxs in patient_to_index.items():
        for i in range(len(idxs)):
            ...
            for j in range(i + 1):
                ...
                visit_key = f'visit_{j}'
                patient_data[patient_id][visit_key] = {
                    'visit_id': visit_id,
                ...
                patient_data_no_label[patient_id][visit_key] = {
                    'visit_id': visit_id,
                    'conditions': conditions,
                    'procedures': procedures,
                    'drugs': drugs   
...
def main():
    ...
    for dataset in datasets:
        ...
        for task in tasks:
            ...
            if dataset == "mimic3":
                with open(f"{out_dir}/patient_{dataset}_{task}.json", "w") as f:
                    json.dump(patient_data, f, indent=4)
            elif dataset == "mimic4":
                with open(f"{out_dir}/patient_{dataset}_{task}_.json", "w") as f:
                    json.dump(patient_data, f, indent=4)
    ...
    for task in tasks:
        with open(f"{out_dir}/patient_mimic4_{task}_.json", "r") as f:
            data = json.load(f)
        ...
        with open(f"{out_dir}/patient_mimic4_{task}.json" , "w") as f:
            json.dump(mimic4_samples, f, indent=4)
            
        print(f"Label 1: {label_1_cnt}")
        print(f"Label 0: {label_0_cnt}")
        
        # with open(f"{out_dir}/patient_mimic4_{task}.json", "r") as f:
        #     data = json.load(f)

        # with open(f"{out_dir}/mimic4_{task}.pkl", "rb") as f: # 
